...
const MD2Surface = React.forwardRef<View, Props>(
	(
		{ style, theme: overrideTheme, ...rest }: Omit<Props, 'elevation'>,
		ref
	) => {
...
const Surface = React.forwardRef<View, Props>(
	(
				...
				return elevation.interpolate({
					...
					outputRange: inputRange.map((elevation) => {
						return colors.elevation?.[
							`level${elevation as MD3Elevation}`
						];
					}),
				});
			}
		...
		if (Platform.OS === 'android') {
			...
			const { margin, padding, transform, borderRadius } =
		...
		const getStyleForShadowLayer = (layer: 0 | 1) => {
			return {
				...
				shadowRadius:
					iOSShadowOutputRanges[layer].shadowRadius[elevation],
			};
		};
