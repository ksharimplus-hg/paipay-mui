...
export const renderAppbarContent = _ref => {
  ...
  return React.Children.toArray(children).filter(child => child != null && typeof child !== 'boolean').filter(child => /* @ts-ignore */
  ...
  renderOnly ? renderOnly.includes(child.type) : child).map((child, i) => {
    if (! /*#__PURE__*/React.isValidElement(child) || ![AppbarContent, AppbarAction, AppbarBackAction, Tooltip].includes( /* @ts-ignore */
    child.type)) {
      return child;
    }
    const props = {
