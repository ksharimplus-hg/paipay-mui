...
const renderAppbarContent = _ref => {
  ...
  return _react.default.Children.toArray(children).filter(child => child != null && typeof child !== 'boolean').filter(child => /* @ts-ignore */
  renderExcept ? !renderExcept.includes(child.type) : child)
  /* @ts-ignore */.filter(child => /* @ts-ignore */
  renderOnly ? renderOnly.includes(child.type) : child).map((child, i) => {
    if (! /*#__PURE__*/_react.default.isValidElement(child) || ![_AppbarContent.default, _AppbarAction.default, _AppbarBackAction.default, _Tooltip.default].includes( /* @ts-ignore */
    child.type)) {
      return child;
    }
    const props = {
